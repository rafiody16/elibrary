    ...
    <div class="col-md-8">
        <div class="card bg-white shadow-md rounded-lg overflow-hidden">
            <div class="card-header">
                <h2 class="text-lg ml-8 mt-2 font-semibold text-gray-800">Data Roles</h2>
                <button wire:click.prevent="addRole"
                    ...
                    wire:target="addRole">   
                    <span wire:loading wire:target="addRole" class="mr-2">
                        ...
                        <tbody>
                            @foreach ($roles as $role)
                                <tr class="border-b hover:bg-gray-100">
                                    <td class="px-6 py-4">{{ $role->name_role }}</td>
                                    <td class="px-6 py-4">{{ $role->description }}</td>
                                    <td class="px-6 py-4 text-center">
                                        <button wire:click="editRole({{ $role->id_role }})"
                                        ...
                                        <button wire:click="deleteRole({{ $role->id_role }})"
            ...
            <div class="relative p-5 border w-96 shadow-lg rounded-md bg-white" @click.away="Livewire.emit('closeModal')">
                <div class="text-center">
                    <h3 class="text-lg leading-6 font-medium text-gray-900 mb-4">
                        {{ $id_role ? 'Edit Role' : 'Create New Role' }}
                    </h3>
                    ...
                    <div class="items-center px-4 py-3">
                        ...
                        @else
                            <button wire:click.prevent="storeRole()"
                                ...
                                wire:loading.attr="disabled"
                                wire:target="storeRole">
                                <span wire:loading wire:target="storeRole" class="mr-2">
                                    <svg class="animate-spin h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                    </svg>
